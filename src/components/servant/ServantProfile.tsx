...
export default function ServantProfile() {
  ...
  const { id } = useParams();
  const manualData = id ? servantTranslations[id] : null;
...
<ServantLoreSection title={manualData?.perfil.title || "Perfil"}>
  ...
  {/* CAIXA 2: FALAS (SÓ APARECE SE TIVERES ESCRITO) */}
{manualData?.falas && (
  <div className="space-y-4">
    {/* Botão de Controle da Secção */}
    <button 
    ...
    {isFalasOpen && (
      <div className="p-4 bg-[#3A3330]/50 rounded-b-lg border-t-0 animate-in fade-in slide-in-from-top-2">
        {manualData.falas.sections.map((section, sIdx) => (
